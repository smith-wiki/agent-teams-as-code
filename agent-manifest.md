...
APM is not required for this group model. For the reuse-first path, use `.github/agents/*.agent.md` as the member files: keep an immutable opaque `metadata.agentId` in each profile (a GitHub annotation, not a portable runtime field), and let Nix project the Markdown profile into the selected harness's native files. With no cross-consumer requirement, native configuration under `agents/<stable-id>/` remains the simpler alternative. The release pipeline derives a deployment index of IDs and **published** image digests after building and pushing images; neither path needs a second hand-maintained membership list ([language choice](agent-language-options.md), [GitHub agent metadata](https://docs.github.com/en/copilot/reference/custom-agents-configuration#yaml-frontmatter-properties), [Nix image builds](https://nixos.org/manual/nixpkgs/stable/#ex-ociTools-buildContainer-bash)).
